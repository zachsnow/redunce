...
type repository struct {
	...
	_cache   Cache
	_file    string   // For backwards compatibility
	_files   []string // Multiple filenames to check (in priority order)
	_exclude GitIgnore
} // repository{}
...
// NewRepositoryWithFiles returns a GitIgnore instance that checks multiple
// ignore filenames at each directory level. Filenames are checked in the order
// provided, with earlier filenames taking precedence. For example:
// NewRepositoryWithFiles(base, []string{".redunceignore", ".gitignore"}, cache, errors)
// will check .redunceignore first, then .gitignore at each directory level.
func NewRepositoryWithFiles(base string, files []string, cache Cache, errors func(e Error) bool) GitIgnore {
	// If no files specified, default to .gitignore
	if len(files) == 0 {
		files = []string{File}
	}

	// Use the first file for backwards compatibility
	return newRepositoryWithFilesInternal(base, files[0], files, cache, errors)
}
...
func NewRepositoryWithCache(base, file string, cache Cache, errors func(e Error) bool) GitIgnore {
	// Use single file for backwards compatibility
	return newRepositoryWithFilesInternal(base, file, []string{file}, cache, errors)
}

func newRepositoryWithFilesInternal(base, file string, files []string, cache Cache, errors func(e Error) bool) GitIgnore {
	...
	return _repository
} // newRepositoryWithFilesInternal()
...
func (r *repository) Absolute(path string, isdir bool) Match {
	...
	if _prefix > len(path) {
		// path is the base directory itself
		return nil
	}
...
func (r *repository) Relative(path string, isdir bool) Match {
	...
	//		- we consider ignore files in the current directory first, then
	//		  move up the path hierarchy
	//		- when multiple filenames are configured (e.g., .redunceignore and .gitignore),
	//		  we check them in order at each directory level, with earlier filenames
	//		  taking precedence
	var _last string
	for {
		// Check each filename at this directory level (in priority order)
		for _, filename := range r._files {
			_file := filepath.Join(r._base, _parent, filename)
